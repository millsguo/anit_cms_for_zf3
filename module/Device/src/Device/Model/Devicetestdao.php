...
class Devicetestdao {
    ...
    public function addDevicetest(Device $device) {

        $requete = $this->dbGateway->prepare("INSERT into deviceregistrationtest
    ...
    public function updateDevicetest(Device $device) {
        $requete = $this->dbGateway->prepare("UPDATE deviceregistrationtest
    ...
    public function getDevicetestToken($deviceType, $token, $userid) {
        ...
        if ($deviceType == 1) {
            $sql = "select device_publicid from deviceregistrationtest where device_userid= :userid LIMIT 1";
        } 
        else {
            $sql = "select device_publicid from deviceregistrationtest where device_devicetoken= :token LIMIT 1";
        }
    ...
    //fonction de creation d'une string aleatoire - 30 caracteres separes
    ...
    protected function isPublicIdExist($publicId) {
        ...
        $sql = "SELECT device_publicid FROM deviceregistrationtest WHERE device_publicid= :publicid;";
        //var_dump($sql);
        $request = $this->dbGateway->prepare($sql)or die(print_r($this->dbGateway->error_info()));
        $request->execute(array(
            'publicid'=>$publicId
