...
final class FileManager
{
	...
	function formatNameFile($string)
	{
	...
	function uploadfiles($fichier,$chemin,$newNameF){
		...
		if($fichier['name']!="")
		{
			...
			$newNameFichier=strtolower($newNameFichier);
			//@unlink($chemin.$newNameFichier);
			if(file_exists($chemin.$newNameFichier)){
				$newNameFichier=time()."_".$newNameFichier;	//Si un autre fichier du meme nom existe, on renomme
			}
                        
                        //print_r($chemin . $newNameFichier);
                        //exit;
                        
			@copy($fichier['tmp_name'], $chemin . $newNameFichier);
                        
		}
		return $newNameFichier;
	}
	...
	function updatefiles($fichier,$old_fichier,$chemin,$newName)
	{
		...
		{	// si pas de fichier uploadé
			...
			if(file_exists($chemin.$old_fichier))	$updatefiles=$old_fichier;		else	$updatefiles="";
	...
	{ 
		...
		if(($this->extractExtension($fichier_image)!="jpg") && ($this->extractExtension($fichier_image)!="jpeg") && ($this->extractExtension($fichier_image)!="png"))	return;
